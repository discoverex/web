...
- **특징**: 사용자는 여기서 로그인하며, 로그인 상태는 모든 하위 게임 앱(`discoverex`, `magic-eye`)으로 원활하게 공유됩니다.
...
- **상태 유지**: 각 앱은 개별 도메인(또는 서브도메인)에서 동작하지만, Firebase의 중앙 집중식 인증 세션과 공유 쿠키 전략을 통해 한 곳에서 로그인하면 모든 앱에서 즉시 인증된 상태로 전환됩니다.
