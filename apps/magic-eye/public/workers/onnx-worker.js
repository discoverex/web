...
async function getSession(modelUrl) {
  ...
  const fileName = modelUrl.includes("/")
    ? modelUrl.split("/").pop()
    : modelUrl;
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(`모델 다운로드 실패 (상태 코드: ${response.status})`);
    const modelBuffer = await response.arrayBuffer();

    console.log(
      `[WORKER] ✅ 다운로드 완료: ${(modelBuffer.byteLength / 1024 / 1024).toFixed(2)}MB`,
    );
    ...
    try {
      ...
      usedBackend = handlerName.toLowerCase().includes("gpu")
        ? "webgpu"
        : "wasm";
    } catch (gpuError) {
      // 시도 2: WebGPU 실패 시 WASM(CPU)으로 강제 재시도
      console.warn(
        "⚠️ [AI 엔진] WebGPU 가속을 사용할 수 없어 WASM 모드로 전환합니다.",
      );
      session = await ort.InferenceSession.create(modelBuffer, {
