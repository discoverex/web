...
export default function GameCanvas() {
  ...
  const [remainingTargets, setRemainingTargets] = useState<Target[]>([
          ...
          {logs.slice().reverse().map((log) => (
            <div key={log.event_id} className="mb-2 border-b border-white/5 pb-1 flex justify-between">
              <span>
                ...
                ID: {log.event_id.slice(0, 8)} | ({Math.round(log.click_coordinate.x)}, {Math.round(log.click_coordinate.y)})
              </span>
